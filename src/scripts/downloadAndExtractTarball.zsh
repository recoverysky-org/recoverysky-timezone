...
# downloadAndExtractTarball.zsh
# Downloads a tarball from URL, extracts it, and copies contents to destination folder
#
# Usage: ./downloadAndExtractTarball.zsh <url> <destination_folder> [tmp_dir] [--dry-run]
...
show_help() {
    cat << EOF
downloadAndExtractTarball.zsh - Download and extract tarball to destination
...
main() {
    ...
    if ! curl -L -f -s -o "$tmp_download_path" "$url"; then
    ...
    local extracted_contents=($(ls -A "$tmp_extract_path"))
    if [[ ${#extracted_contents[@]} -eq 0 ]]; then
        log_error "No contents found in extracted tarball"
        cleanup "$tmp_dir" "false"
        exit 5
    fi
    
    log_info "Found ${#extracted_contents[@]} items in extracted tarball"
    
    # Determine what to copy
    local source_path="$tmp_extract_path"
    if [[ ${#extracted_contents[@]} -eq 1 ]]; then
        local first_item="$tmp_extract_path/${extracted_contents[1]}"
...
trap 'cleanup "$tmp_dir" "false"' EXIT
